...
local jaan_details = {
    ...
    unit_list = "snek_hkrul_mar_landship,hkrul_privateers,hkrul_privateers_dual_swords,wh2_dlc11_cst_inf_sartosa_militia_0,hkrul_mar_inf_boogschutter,hkrul_carriers,hkrul_burgher,hkrul_burgher", -- unit keys from main_units table
    region_key = "",
    ...
    core:add_listener(
        ...
        function(context)
            local character = context:character()
            local faction = character:faction()
            return character:character_subtype("hkrul_pg") and character:rank() >= 6 and faction:ancillary_exists("hkrul_mar_anc_araby") == false
        end,
        function(context)
            cm:force_add_ancillary(
                context:character(),
