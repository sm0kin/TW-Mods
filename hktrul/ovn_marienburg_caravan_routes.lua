...
local event_table = {
	...
	function(event_conditions,caravan_handle)
		...
		local attacking_force = hkrul_mar_generate_attackers(bandit_threat, "");
		...
		if target_faction_object:subculture()=="wh3_main_sc_ogr_ogre_kingdoms" and not target_faction_object:name() == "wh3_main_ogr_ogre_kingdoms_qb1" then
			...
			out.design("Use best ogre faction "..hkrul_mar_get_best_ogre_faction(own_faction:name()):name() )
			payload_builder:diplomatic_attitude_adjustment(hkrul_mar_get_best_ogre_faction(own_faction:name()), 6);
		end
	...
	function(event_conditions,caravan_handle)
		...
		if caravan:caravan_master():character_details():has_skill("wh3_main_skill_cth_caravan_master_scouts") or cm:random_number(3,0) == 3 then
			...
			hkrul_mar_spawn_caravan_battle_force(caravan, attacking_force, target_region, true, true);
		end;
	end,
	true},
	...
	{function(world_conditions)
		...
		if caravan_force_unit_list:num_items() > 1 then
			...
			if random_unit == "hkrul_mar_caravan_master" then --or random_unit == "wh3_main_cth_cha_lord_magistrate_0" then
				random_unit = "NONE";
			end
	...
	function(event_conditions,caravan_handle)
		...
		local bandit_threat = tonumber(decoded_args[3]);
		local attacking_force = hkrul_mar_generate_attackers(bandit_threat,"ogres")
	...
	{function(world_conditions)
		...
		if caravan_force_unit_list:num_items() > 1 then
			...
			if random_unit == "hkrul_mar_caravan_master" then
				random_unit ="NONE";
			end
	...
	function(event_conditions,caravan_handle)
		...
		local bandit_threat = tonumber(decoded_args[3]);
		local attacking_force = hkrul_mar_generate_attackers(bandit_threat,"daemon_incursion")
...
cm:add_first_tick_callback_new(
	function()
		...
		for i=0, all_factions:num_items()-1 do
			...
			if not faction:is_human() and faction:name() == "wh_main_emp_marienburg" then
...
core:add_listener(
	...
	function(context)
		...
		for i=0, caravan_force_list:num_items()-1 do
			...
			if unit:unit_key() == "hkrul_mar_caravan_master" then
...
function hkrul_mar_ivory_road_event_handler(context)
	...
	for i = 0,num_regions-1 do
		...
		if not reg:is_null_interface() then
			...
			out.design("null_interface (sea region?)")
		end;
	end
...
function hkrul_mar_recruit_starter_caravan()
	...
	for i=0, faction_list:num_items()-1 do
		...
		if faction:is_human() and faction:name() == "wh_main_emp_marienburg" then
			...
			if available_caravans:is_empty() then
				...
				--[[
				core:add_listener(
					"hkrul_mar_starter_caravan_skills_"..tostring(faction:name()),
					"CaravanRecruited",
					function(context)
	...
	local end_nodes = {}
			...
			if cm:get_campaign_name() == "main_warhammer" then
				end_nodes = {
					...
					["wh3_main_combi_region_clar_karond"]				=75-cm:random_number(50,0),
...
function hkrul_mar_build_list_of_nodes()
...
function hkrul_mar_get_random_ogre_faction()
	local factions = cm:model():world():faction_list();
	local faction = nil;
	
	out.design("call hkrul_mar_get_random_ogre_faction()")
...
function hkrul_mar_get_best_ogre_faction(self_faction)
	...
	out.design("call hkrul_mar_get_best_ogre_faction()")
	if not factions:is_empty() then
		for i=0, factions:num_items() -1 do
			faction = factions:item_at(i);
			if faction:subculture()=="wh3_main_sc_ogr_ogre_kingdoms" and faction:name()~="wh3_main_ogr_ogre_kingdoms_qb1" then
