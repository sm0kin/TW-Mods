...
local function confed_revived(confederator, confederated)
    ...
    -- pre confed chivalry
    if confederator:pooled_resource_manager():resource("brt_chivalry"):is_null_interface() == false then --confederator:subculture() == "wh_main_sc_brt_bretonnia"
    ...
    for i = 0, char_list:num_items() - 1 do 
        ...
        if is_character(char) then
            sm0_log("Faction: "..confederator:name().." | ".."Character cqi: "..command_queue_index.." | ".."Subtype: "..char:character_subtype_key().." | Forename: "
    ...
    core:add_listener(
        ...
        function(context)
            ...
            if confederator:pooled_resource_manager():resource("wh3_main_ksl_devotion"):is_null_interface() == false then --confederator:subculture() == "wh3_main_sc_ksl_kislev"
            ...
            if confederator:pooled_resource_manager():resource("brt_chivalry"):is_null_interface() == false then --confederator:subculture() == "wh_main_sc_brt_bretonnia"
                ...
                sm0_log("Previous chivalry: "..tostring(previous_chivalry).." | Current chivalry: "..tostring(current_civalry))
            ...
            for i = 0, char_list:num_items() - 1 do 
                ...
                if is_character(char) and (cm:char_is_agent(char) or cm:char_is_general(char)) and not table.contains(pre_confed_char_list, current_cqi) then
                    ...
                    if faction_leader_cqi == nil and cm:char_is_general(char) then
                        faction_leader_cqi = current_cqi -- faction leader has the lowest cqi of all newly spawned characters
                    end
                    if refugee_types_value == "immortal" or (current_cqi ~= faction_leader_cqi and refugee_types_value == "faction_leader") then 
                        ...
                        elseif char:character_details():is_unique() then
                            ...
                            sm0_log("MORTAL CHARACTER | Faction: "..confederated:name().." | ".."Character cqi:"..current_cqi .." | subtype: "
                            ..common.get_localised_string(char:character_subtype_key()).." | Forename: "
                            ..common.get_localised_string(char:get_forename()).." | Surname: "..common.get_localised_string(char:get_surname()))                                
                        end
...
local function init_recruit_defeated_listeners(enable_value)
    ...
    core:remove_listener("recruit_defeated_sought_refuge_FactionTurnStart")
    ...
    --core:remove_listener("recruit_defeated_nakai_horde_MilitaryForceCreated") -- wh3 8.1: Nakai should now keep his horde buildings after he gets confederated by another Lizardmen faction.

	if enable_value then
        -- wh3 8.1: Nakai should now keep his horde buildings after he gets confederated by another Lizardmen faction.
        --core:add_listener( 
        ...
        core:add_listener(
            "recruit_defeated_sought_refuge_FactionTurnStart",
            "FactionTurnStart",
            true,
            function(context)
                if not (cm:model():turn_number() == 1 and context:faction():is_human()) then 
        ...
        core:add_listener(
            ...
            function(context)
                    ...
                    and not cm:get_saved_value("rd_choice_2_"..current_faction:name()) then 
                        ...
                        or (include_seccessionists_value and (not is_faction_exempted(current_faction) or is_faction_seccessionist(current_faction))) then --and not current_faction:subculture() == "wh_dlc08_sc_nor_norsca"
                            ...
                            if prefered_faction and ((prefered_faction:is_human() and player_confederation_limit > 1) or not table.contains(subcultures_accepting_refugees, prefered_faction:subculture())) then
                                if prefered_faction:is_human() and not prefered_faction:is_idle_human() then
                ...
                elseif #dilemmas < 1 and #ai_confeds >= 1 then
                    ...
                    sm0_log("#dilemmas < 1 and #ai_confeds >= 1 | "..confederator:name().." | "..confederated:name())
                    core:add_listener(
