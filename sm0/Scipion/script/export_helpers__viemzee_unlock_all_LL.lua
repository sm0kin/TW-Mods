...
function v_faction_defeated(current_faction)
...
function v_unlock_ll(lord_subtype, current_faction)
			...
			for subculture, data in pairs(LEGENDARY_LORDS) do
				...
				if fac_subculture == subculture then
					...
					if LEGENDARY_LORDS[subculture] ~= nil then
						...
						if vfs.exists("script/export_helpers__viemzee_unlock_all_mixu.lua") then
						--if cm:get_saved_value("v_unlock_all_mixu") then
							for j = 1, #LEGENDARY_LORDS[subculture].mixu_lords do
								...
								if not(fac_name == "wh2_main_def_har_ganeth" and LEGENDARY_LORDS[subculture].mixu_lords[j] == "def_tullaris_dreadbringer") then 
...
local function init()
	...
	--cm:transfer_region_to_faction("wh2_main_charnel_valley_granite_massif", "wh2_dlc09_tmb_khemri")



	v_log("Unlock all Version 1.21")
	
	--if not cm:get_saved_value("v_meta_unlock") then
		--cm:set_saved_value("v_meta_unlock", false)
	--end

	--if not cm:get_saved_value("v_unlock_all_mixu") then
	--	cm:set_saved_value("v_unlock_all_mixu", false)
	--end
