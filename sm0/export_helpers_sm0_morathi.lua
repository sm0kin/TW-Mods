--AI
--effect_bundles test
cm:set_saved_value("sm0_morathi", true);
...
local charSubmorathi = "wh2_main_def_morathi";
...
function getMazdaCharacterByFaction()
	...
	if localFaction then
		...
		for i = 0, characterList:num_items() - 1 do
			...
			if currentChar:character_subtype(charSubmorathi) then
				return currentChar;
			end;
		end;
	end;
...
function replaceLoreEffect(name, char) -- test
	...
	name = "loreButtonTest"; --test
	name = string.lower(name);
	local effectBundle = string.gsub(name, "lorebutton", "wh2_sm0_effect_bundle_");
            ...
            function(context)
                for _, otherButton in ipairs(buttons) do
                    if button.name == otherButton.name then
						...
						--replaceLoreEffect(otherButton.name, char);
...
function getMazdaCharacter()
	...
	if not cm:model():pending_battle():is_active() then
		...
		if char == nil then
			...
			if string.find(focusButtonText,"morathi") then
	...
	elseif cm:model():pending_battle():attacker():faction():is_human() then
		...
		if not char:character_subtype(charSubmorathi) and cm:model():pending_battle():secondary_attackers():num_items() >= 1 then
			local attackers = cm:model():pending_battle():secondary_attackers();
			for i = 0, attackers:num_items() - 1 do
				if attackers:item_at(i):character_subtype(charSubmorathi) then
	...
	elseif cm:model():pending_battle():defender():faction():is_human() then
		...
		if not char:character_subtype(charSubmorathi) and cm:model():pending_battle():secondary_defenders():num_items() >= 1 then
			local defenders = cm:model():pending_battle():secondary_defenders();
			for i = 0, defenders:num_items() - 1 do
				if defenders:item_at(i):character_subtype(charSubmorathi) then
...
function createLoreButton1() -- character_panel
	cm:callback(
		function(context)
			...
			if string.find(namePanelText, "morathi") then
...
function playerLoreListener()
	...
	if buttonLocation2 then
		core:add_listener(
			...
			function(context)
				...
				if selectedCharacter:character_subtype(charSubmorathi) then --and lordUnitPanel:CurrentState() == "selected_hover"
					createLoreButton2();	
				end			
			end,
			true
	...
	if buttonLocation3 then		
		core:add_listener(
			...
			function(context)
				...
				if pb:battle_type() ~= "ambush" and selectedCharacter:character_subtype(charSubmorathi) then
					createLoreButton3();
				end
			end,
			true
	...
	if buttonLocation4 then
		core:add_listener(
			...
			function(context)
				local selectedCharacter = getMazdaCharacter();
				if selectedCharacter:character_subtype(charSubmorathi) then
					createLoreButton4();	
				end
			end,
			true
		...
		core:add_listener(
			...
			function(context)
				if context:character():character_subtype(charSubmorathi) and loreBookButton4 == nil then
					createLoreButton4();
				elseif context:character():character_subtype(charSubmorathi) ~= true and loreBookButton4 ~=nil then
					closeLoreUI4();
				end
			end,
			true
...
function aiLoreListener()
	core:add_listener(
		...
		function(context)
			...
			return pb:attacker():character_subtype(charSubmorathi) and not pb:attacker():faction():is_human() or pb:defender():character_subtype(charSubmorathi) and not pb:attacker():faction():is_human();
		end,
		function(context)
			local charMazda = nil;
			local enemyFaction = nil;
			if context:pending_battle():attacker():character_subtype(charSubmorathi) then
			...
			local tableSize = nil;
			local roll = nil;
			local enableaAdvantageousRoll = false;
			if cm:random_number() == 1 and enableaAdvantageousRoll then
...
if cm:get_saved_value("wec_ll_revival") then
	core:add_listener(
		...
		"CharacterCreated", 
		function(context) 
			return context:character():character_subtype(charSubmorathi);
		end,
		function(context)
