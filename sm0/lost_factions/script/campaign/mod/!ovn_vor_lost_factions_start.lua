...
local function albion_setup()
	...
	cm:force_religion_factors("wh2_main_vor_albion_albion", "wh_main_religion_untainted", 0.75, "wh_main_religion_chaos", 0.25)
...
local function grudgebringers_setup()
	...
	cm:add_unit_to_faction_mercenary_pool(faction_name, "wh2_dlc13_emp_cav_pistoliers_1_imperial_supply", 1, 0, 5, 0, xp_level, frr, srr, trr, true);
	cm:add_unit_to_faction_mercenary_pool(faction_name, "ragnar_wolves", unit_count, rcp, max_units, murpt, xp_level, frr, srr, trr, true);
	cm:add_unit_to_faction_mercenary_pool(faction_name, "grudgebringer_infantry", unit_count, rcp, max_units, murpt, xp_level, frr, srr, trr, true);
	cm:add_unit_to_faction_mercenary_pool(faction_name, "grudgebringer_cannon", unit_count, rcp, max_units, murpt, xp_level, frr, srr, trr, true);
	cm:add_unit_to_faction_mercenary_pool(faction_name, "grudgebringer_crossbow", unit_count, rcp, max_units, murpt, xp_level, frr, srr, trr, true);
	cm:add_unit_to_faction_mercenary_pool(faction_name, "treeman_gnarl_fist", unit_count, rcp, max_units, murpt, xp_level, frr, srr, trr, true);
	cm:add_unit_to_faction_mercenary_pool(faction_name, "urblab_rotgut_mercenary_ogres", unit_count, rcp, max_units, murpt, xp_level, frr, srr, trr, true);
	cm:add_unit_to_faction_mercenary_pool(faction_name, "galed_elf_archers", unit_count, rcp, max_units, murpt, xp_level, frr, srr, trr, true);
	...
	cm:spawn_character_to_pool("wh2_dlc13_emp_the_huntmarshals_expedition", "names_name_999982320", "", "", "", 18, true, "general", "ovn_hlf_ll", false, "")
end
...
local function dreadking_setup()
	...
	if dread_king:is_human() then	
		...
		cm:region_slot_instantly_upgrade_building(galbaraz_region:settlement():active_secondary_slots():item_at(1), "wh2_main_special_galbaraz_mourkain_vmp_1");
		cm:region_slot_instantly_upgrade_building(galbaraz_region:settlement():active_secondary_slots():item_at(2), "wh_main_vmp_walls_1");
	else
		...
		cm:region_slot_instantly_upgrade_building(galbaraz_region:settlement():active_secondary_slots():item_at(1), "wh2_main_special_galbaraz_mourkain_tmb_1")	
		cm:region_slot_instantly_upgrade_building(galbaraz_region:settlement():active_secondary_slots():item_at(2), "wh2_dlc09_tmb_defence_major_2");
	end;
		
	local pyramid_region = cm:model():world():region_manager():region_by_key("wh2_main_vor_great_mortis_delta_black_pyramid_of_nagash")
	cm:region_slot_instantly_dismantle_building(pyramid_region:settlement():active_secondary_slots():item_at(0))
	
	cm:override_building_chain_display("wh_main_SAVAGEORC_settlement_major", "wh_main_VAMPIRES_settlement_major", "wh2_main_vor_southern_badlands_galbaraz")
