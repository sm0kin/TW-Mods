local function sr_chaos()	
	if cm:model():campaign_name("main_warhammer") then
		...
		if cm:is_new_game() then
			if cm:get_faction("wh_dlc08_nor_naglfarlings"):is_human() then
			...
			---- Start Event Message Pop Up 	
					...
					"FactionRoundStart", 
					function(context) return context:faction():is_human() and cm:model():turn_number() == 15 end,
					function() chshordestartmess() end,
					false 
					);
			...
			---- Chaos Army Spawn Script listener
					...
					"FactionRoundStart", 
					function(context) return context:faction():name() == "wh_dlc08_nor_naglfarlings" and cm:model():turn_number() > 16 end,
					function() chshordespawn() end,
					true 
					);
...
----  CWD HOLD KRADTOMMEN - Settlement 1
	...
	if kradtommen_region:owning_faction():name() == "wh_dlc08_nor_naglfarlings"  then
		...
		if 1 == cm:random_number(75, 1) then
			...
			cm:force_diplomacy("faction:wh_dlc08_nor_naglfarlings", "faction:wh2_main_chs_chaos_incursion_hef", "military alliance", false, false, true);
...
----  CWD HOLD Mordheim - Settlement 2
		...
		----  Sylvania Spawn: Option 2A
			if 3 == cm:random_number(75, 1) then	
				...
				cm:force_diplomacy("faction:wh_dlc08_nor_naglfarlings", "faction:wh2_main_chs_chaos_incursion_def", "military alliance", false, false, true);
...
----  CWD HOLD Xahutec - Settlement 4
		if xahutec_region:owning_faction():name() == "wh_dlc08_nor_naglfarlings"  then		
			...
			----  Xauhutec Spawn: Option 4A
				...
				cm:force_diplomacy("faction:wh_dlc08_nor_naglfarlings", "faction:wh2_main_chs_chaos_incursion_lzd", "military alliance", false, false, true);
...
function setup_cwd_and_fimir_raze_region_monitor() -- Applies Chaos corruption via an effect bundle to a region that is razed by an army belonging to CWD & Fimir
	core:add_listener(
		...
		function(context) return context:character():faction():subculture() == "wh_main_sc_nor_warp" or context:character():faction():subculture() == "wh_main_sc_nor_fimir" end,
		function(context)
...
cm:add_first_tick_callback(function() sr_chaos() end)		